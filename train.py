...
def main():
    ...
    # 确定resume参数（优先级：命令行 > 配置文件 > 硬编码）
    resume_epoch = None
    if args.resume is not None:
        # 命令行参数优先级最高
        resume_epoch = args.resume
        print(f"🔄 使用命令行resume参数: {resume_epoch}")
    else:
        # 检查配置文件中的resume参数
        config_resume = config.get_checkpoint_config().get('resume')
        if config_resume is not None:
            resume_epoch = config_resume
            print(f"🔄 使用配置文件resume参数: {resume_epoch}")
        else:
            print("🆕 未指定resume，将创建新目录开始训练")
    ...
    try:
        ...
        # 如果使用resume模式，先检查checkpoint文件
        if resume_epoch is not None:
            print(f"🔄 Resume模式：从epoch {resume_epoch}继续训练")
            if os.path.exists(result_dir):
                ...
                if os.path.exists(ckpt_dir):
                    ...
                    # 检查指定的epoch是否存在
                    target_checkpoint = f"checkpoint_epoch_{resume_epoch}.pth"
                    if target_checkpoint in checkpoint_files:
                        print(f"✅ 找到目标checkpoint: {target_checkpoint}")
                    else:
                        print(f"❌ 错误：指定的epoch {resume_epoch}的checkpoint不存在")
                        print(f"可用的checkpoint: {checkpoint_files}")
                        sys.exit(1)
                else:
        ...
        # 如果使用resume模式，将resume_epoch传递给训练函数
        if resume_epoch is not None:
            print(f"📋 Resume信息将传递给训练函数: epoch {resume_epoch}")
    ...
    try:
        ...
        if args.resume is not None:
            print(f"  # 或者使用原始脚本继续训练:")
            print(f"  python models/{args.model}/UCF101_{args.model}_fixed.py --resume {args.resume}")
        sys.exit(1)
